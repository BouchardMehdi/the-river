...
export class AuthService {
  ...
  private tokenSalt(): string {
    const salt = this.config.get<string>('AUTH_TOKEN_SALT') || this.config.get<string>('JWT_SECRET');
    if (!salt) throw new Error('AUTH_TOKEN_SALT or JWT_SECRET is required');
    return salt;
  }
  ...
  private async passwordMatches(stored: string, provided: string): Promise<boolean> {
    ...
    const allowLegacyPlaintext =
      String(this.config.get<string>('AUTH_ALLOW_PLAINTEXT_PASSWORDS') ?? 'false').toLowerCase() === 'true';
    return allowLegacyPlaintext && stored === provided;
  }
